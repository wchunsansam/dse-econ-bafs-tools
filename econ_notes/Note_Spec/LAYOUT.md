...
學生版（課堂代碼 HTMS）：ECON 各年級筆記顯示「完整」與「填空」，可切去完整版。點揭與書本答案仍只給老師（bteacher，大小寫皆可）。BAFS 學生版維持只有填空。
...
PDF 課本練習是**另一頁**（`pdf_mark.html`），不是把 PDF 嵌進筆記。筆記頂欄只放連結。該頁用與作業角老師批改相同的畫筆台：畫筆／直線／圓、擦膠、放大、上一頁／下一頁。筆跡存在這部裝置（「儲存筆記」），也可下載含筆跡 PDF。教師頂欄可用「書本練習／書本答案」直接來回切；繁／EN 會改開同一份的對應語文 PDF。學生可在書本練習上書寫並儲存，但看不到答案鈕。
...
| `.grid3` | 三格並排分類（窄屏變單欄）；題目用 `.ttl`，補充用 `.hint` |
| `.grid4` | 2×2 分類（窄屏變單欄） |

窄於筆記容器約 720px 時：`.two` `.illo` `.grid3` `.grid4` `.glossary` `.meta` 收成單欄。不要為某一章寫死寬度。
...
SVG 供需圖：曲線平移用**水平**箭嘴（line + 獨立三角箭頭，同一 y），放在兩線中間的空隙，不要貼在終點曲線上、也不要用斜向箭頭。軸上的變動箭嘴放在刻度標籤**下方空隙**（不要與 Yf／Y₁／M₀ 同一基線），標籤分置左右兩側，箭嘴只走中間，不要穿過文字。AD₂ 不要用近橙紅（會跟 SRAS 撞色）；平移／軸箭嘴用深色（#0f172a），不要跟終點曲線同色。緊縮 AD 用紫（#7c3aed），SRAS 用橙（#ea580c）。
