(function () {
  ...
  function installSecretFieldStyle() {
    if (document.getElementById("htms-secret-style")) return;
    const s = document.createElement("style");
    s.id = "htms-secret-style";
    s.textContent = [
      'input[type="password"],input.htms-secret{-webkit-text-security:disc;text-security:disc;',
  ...
  const SECRET_SEL = 'input[type="password"], input.htms-secret';
  let secretObs = null;

  function noFillAttrs(el) {
  ...
  function hardenSecretField(el) {
    if (!el || el.dataset.htmsNoFill === "1") return;
    el.dataset.htmsNoFill = "1";
    noFillAttrs(el);
    el.readOnly = true;
    const arm = function () { el.readOnly = false; };
    el.addEventListener("focus", arm);
    el.addEventListener("pointerdown", arm);
    if (document.activeElement === el) arm();
  ...
  function hardenSecretInputs(root) {
    const scope = root && root.querySelectorAll ? root : document;
    scope.querySelectorAll(SECRET_SEL).forEach(hardenSecretField);
    scope.querySelectorAll('input[autocomplete="username"], input[autocomplete="current-password"], input[autocomplete="new-password"]').forEach(noFillAttrs);
  }

  function clearSecretInputs(root) {
    const scope = root && root.querySelectorAll ? root : document;
    scope.querySelectorAll(SECRET_SEL).forEach(function (el) { el.value = ""; });
  }

  function observeSecretInputs() {
    if (secretObs || !window.MutationObserver || !document.body) return;
    secretObs = new MutationObserver(function () { hardenSecretInputs(document); });
    secretObs.observe(document.body, { childList: true, subtree: true });
  }
  ...
  window.HTMSGate = {
    ...
    refreshThisPage, hardenSecretInputs, clearSecretInputs
  };

  hideAppUntilOk();
  installSecretFieldStyle();
  installPwa();
  enforce();
  whenReady(function () {
    installRefreshBar();
    hardenSecretInputs(document);
    observeSecretInputs();
  });
