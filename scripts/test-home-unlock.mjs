...
assert.ok(/id="unlock-code"[^>]*type="password"/.test(html), "class code uses native password masking");
assert.ok(!main.includes("bindUnlockMask") && !main.includes("unlockSecret"), "custom star mask removed");
assert.ok(gate.includes("installSecretFieldStyle") && gate.includes("-webkit-text-security:disc"), "password fields keep disc masking");
assert.ok(gate.includes("function hardenSecretInputs") && gate.includes("function clearSecretInputs"), "secret fields refuse autofill");
assert.ok(html.includes('method="post"') && html.includes('autocomplete="off"'), "unlock forms do not advertise a login");
assert.ok(!html.includes('autocomplete="current-password"') && !html.includes('autocomplete="username"'), "homepage does not request saved passwords");
...
assert.ok(gate.includes("installRefreshBar()") && gate.includes("whenReady(function"), "refresh bar mounts after the page body exists");
