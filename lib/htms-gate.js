(function () {
  ...
  function enforce() {
    ...
    if (!unlocked()) {
      hideAppUntilOk();
      if (isBafsPastPaperFile()) location.replace(homeUrl() + "&pp=1");
      else if (isEconPastPaperFile()) location.replace(homeUrl() + "&ppe=1");
      else location.replace(homeUrl());
      return;
    }
    if (isBafsPastPaperFile() && !ppUnlocked()) {
      hideAppUntilOk();
      location.replace(homeUrl() + "&pp=1");
      return;
    }
    if (isEconPastPaperFile() && !econPpUnlocked()) {
      hideAppUntilOk();
  ...
  window.HTMSGate = {
    ...
    econPpUnlocked, econPpUnlock, econPpLock, isEconPastPaperFile,
